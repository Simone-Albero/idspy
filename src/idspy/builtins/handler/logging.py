...
from ...core.events.handler import BaseHandler
...
class DataFrameProfiler(BaseHandler):
    ...
    def handle(self, event: Event) -> None:
        ...
        try:
            ...
            self.logger.log(
                self.level,
                "DATAFRAME source=%s shape=(%s,%s) mem=%s nans=%s dtypes=%s",
                event.source,
                rows,
                cols,
                _fmt_bytes(mem),
